...
#[test]
fn scrub_cli_scans_compacted_db() -> anyhow::Result<()> {
    let dir = TempDir::new()?;
    ...
    let output = Command::new(layerdb_bin()?)
        .args(["scrub", "--db", dir.path().to_str().expect("utf8 path")])
        .output()?;

    assert!(
        output.status.success(),
        "scrub failed: stdout={} stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );

    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("scrub: level="), "stdout={stdout}");
